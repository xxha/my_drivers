...
static int scull_p_faysnc(int fd, struct file *filp, int mode);
...
static unsigned int scull_p_poll(struct file *filp, poll_table *wait)
{
	...
	if (dev->rp != dev->wp)
		mask |= POLLIN | POLLRDNROM; /* readable */
	if (spacefree(dev))
...
int scull_p_init(dev_t firstdev)
{
	...
	if (result < 0) {
		printk(KERN_NOTIC "Unable to get scullp region, error %d\n", result);
		return 0;
	}
