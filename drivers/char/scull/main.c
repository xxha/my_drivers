...
/* Module Parameters which can be set at load time */
...
/*
 * Create a set of operations for our proc file.
 */
...
static void scull_remove_porc(void)
{
...
ssize_t scull_read(struct file *filp, char __user *buf, size_t count, 
			loff_t *f_pos)
{
	struct scull_dev *dev  = filp->private_data; /* assigned by scull_open*/
...
int scull_open(struct inode *inode, struct file *filp)
{
	...
	if ((filp->f_flags & O_ACCMODE) == O_WRONLY) {
		if (mutex_lock_interruptable(&dev->mutex))
			return -ERESTARTSYS;
